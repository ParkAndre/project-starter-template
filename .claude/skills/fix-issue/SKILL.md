...
2. For each AC in planned sequence, run the RED → Verify RED → GREEN → REFACTOR loop **as defined in `.claude/skills/tdd/SKILL.md`** — read it and follow it; the Iron Law applies (NO production code without a failing test first; a test that passes immediately is fake — rewrite it)
3. Commit after each phase in SIMPLE format (feature branch WIP, no `type:` prefix): `Add failing test for <AC>` / `Implement <AC>` / `Refactor <AC area>`
4. Mark task `completed` when AC's GREEN passes
...
**d. AI-attribution scan** (patterns in `.claude/scan-patterns.md`):
- AI-attribution greps over commit messages (`git log <base>..HEAD --format='%B'`) and the code diff
- If found in commit messages: do NOT use `git rebase -i`; `git commit --amend` with a clean message ONLY if the user explicitly approves amending the immediately previous commit (same rule as `/commit` and `/merge`); otherwise create a clean commit with corrections
...
After choice (a): execute the squash merge **as defined in `.claude/skills/merge/SKILL.md`** — read it and follow its full process (pre-merge gates, fetch + rebase with conflict STOP, re-run tests after rebase, FULL conventional commit with `Closes #<n>`, AI-attribution sweep, push, branch cleanup, verify issue closed). Do not re-derive the steps here; `/merge` is the single source of truth for merging.

### 12. Context handoff (invoked at any point)

If context approaches >70% capacity at any step:
- Create directory if needed: `.planning/` (at the project repo root)
- Write `.planning/handoff-issue-<n>.md` containing:
...
- Report to user: "Context approaching limit. Suggest `/clear` and resume from `.planning/handoff-issue-<n>.md` in a fresh session."
