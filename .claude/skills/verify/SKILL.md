...
|---|---|---|
| Test runner | `package.json` scripts.test, `pytest.ini`, `pyproject.toml`, `phpunit.xml`, `go.mod`, `Cargo.toml`, `Makefile` test target | `npm test` / `pytest` / `vendor/bin/phpunit` / `go test ./...` / `cargo test` / `make test` |
| Linter | `package.json` scripts.lint, `.eslintrc*`, `ruff.toml`, `pyproject.toml [tool.ruff]`, `.flake8`, `phpcs.xml`, `.golangci.yml`, `Cargo.toml` clippy | `npm run lint` / `ruff check` / `flake8` / `phpcs` / `golangci-lint run` / `cargo clippy` |
...
Always-on gates (secrets, AI-attribution, debug-code) run regardless of mode.

### 4. Run always-on safety gates (sequential, all run even on failure)

**4a — Secrets scan** (over `git diff --cached` + `git diff` — staged + unstaged additions):

Patterns (from `/commit` skill):
- `BEGIN (RSA |EC |OPENSSH |DSA )?PRIVATE KEY`
- `AKIA[0-9A-Z]{16}` (AWS)
- `sk_(live|test)_[a-zA-Z0-9]{24,}` (Stripe)
- `ghp_[a-zA-Z0-9]{36}` (GitHub PAT)
- `xox[baprs]-[a-zA-Z0-9-]+` (Slack)
- Generic: `(?i)(password|api[_-]?key|secret|token|access_key)\s*=\s*["'][^"'$]{8,}`
...
**4b — AI-attribution scan:**

- Commit messages: `git log <base>..HEAD --format='%B' 2>/dev/null | grep -iE 'co-authored-by|generated with|claude code|🤖|🎯|Anthropic|Assisted-By'`
- Code diff: `git diff --cached | grep -iE 'co-authored-by.*claude|generated with.*claude|🤖|# written by ai|// generated by ai'`

Match = **MAJOR FAIL** (must be cleaned before PR per CLAUDE.md).

**4c — Debug code scan** (modified non-test source files):

Patterns: `console\.log\(`, `console\.dir\(`, `\bdebugger\b`, `\bdd\(`, `\bvar_dump\(`, `\bprint_r\(`, `\bbreakpoint\(\)`, `\bbinding\.pry\b`
